"""Write-once evaluation runner for the frozen AD01-AD08 role-gap v8 study.

The production pipeline never imports this module. This file intentionally
snapshots the already-tested AC execution design instead of widening the
consumed AC runner to accept a second cohort. Each AD case spends one anonymous
OpenAlex anchor request, records a candidate-local role decision, and may then
spend one already-frozen closure request. The CLI defaults to a zero-network
protocol check; a provider run requires a separate owner authorization naming
the merged revision and frozen fixture.
"""
...
_CASE_ORDER = tuple(f"AD{index:02d}" for index in range(1, 9))
...
class OpenAlexRoleGapEvaluationError(ValueError):
    """Raised when the frozen AD evaluation boundary cannot be represented."""


# Keep the internal name used by the snapshotted kernel local to this module.
# The public AD entrypoint and tests use the evaluation-specific name above;
# retaining the alias keeps cohort-bearing changes easy to audit.
OpenAlexRoleGapLiveError = OpenAlexRoleGapEvaluationError
...
class RoleGapFrozenCaseArtifact(BaseModel):
    """Complete deterministic AD expansion persisted before provider work."""
...
class RoleGapManifestArtifact(BaseModel):
    ...
    mode: Literal["openalex_adaptive_role_gap_v8_ad_manifest"] = (
        "openalex_adaptive_role_gap_v8_ad_manifest"
    )
    ...
    def _validate_frozen_contract(self) -> "RoleGapManifestArtifact":
        ...
        if tuple(case.spec.case_id for case in self.cases) != _CASE_ORDER:
            raise ValueError("manifest cases must remain ordered AD01 through AD08")
...
class RoleGapLaneExecution(BaseModel):
    ...
    case_id: str = Field(pattern=r"^AD0[1-8]$")
    lane_id: LaneId
    ...
    trace_id: str = Field(
        pattern=r"^openalex-role-gap-v8-ad0[1-8]-(?:anchor-search|role-closure-[a-z0-9-]+)$"
    )
...
class RoleGapExecutionArtifact(BaseModel):
    ...
    mode: Literal["openalex_adaptive_role_gap_v8_ad_execution"] = (
        "openalex_adaptive_role_gap_v8_ad_execution"
    )
    cohort: Literal["unseen"] = "unseen"
    production_connected: Literal[False] = False
    report_workflow_connected: Literal[False] = False
    ...
    def _validate_totals_and_states(self) -> "RoleGapExecutionArtifact":
        ...
        if route_case_ids != _CASE_ORDER[: len(route_case_ids)]:
            raise ValueError("route executions must preserve the AD prefix")
        if tuple(item.case_id for item in self.portfolios) != _CASE_ORDER[
            : len(self.portfolios)
        ]:
            raise ValueError("case portfolios must preserve the AD prefix")
        ...
        for case_index, case_id in enumerate(_CASE_ORDER):
            ...
            if not case_lanes:
                if any(item[0] in _CASE_ORDER[case_index + 1 :] for item in lane_pairs):
                    raise ValueError("lane executions skipped an earlier AD case")
                continue
...
def protocol_dry_run() -> dict[str, Any]:
    """Expose frozen AD identities while constructing no provider client."""

    result = dry_run("unseen")
    result["implementation_sha256"] = verify_frozen_implementation()
    result["runner_sha256"] = _file_sha256(_RUNNER_PATH)
    result["evaluation_runner_maximum_requests"] = MAXIMUM_REQUESTS
    result["evaluation_runner_maximum_soft_stop_usd"] = MAXIMUM_SOFT_STOP_USD
    return result
...
def _validate_route_manifest_binding(
    ...
    """Reject a valid route object bound to the wrong frozen AD case."""
...
def execute_unseen_study(
    *,
    ...
    """Run only AD01-AD08 under the frozen adaptive write-once boundary."""

    # Type hints do not enforce the cohort at runtime. Keep this explicit so an
    # accidental development replay fails before output reservation or adapter
    # construction, rather than silently consuming the already-observed AC set.
    if cohort != "unseen":
        raise OpenAlexRoleGapEvaluationError(
            "adaptive role-gap v8 AD entrypoint accepts only the unseen cohort"
        )
    ...
    fixture_sha256, challenge, cases = load_frozen_cases("unseen")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Frozen OpenAlex adaptive role-gap v8 AD evaluation harness."
    )
