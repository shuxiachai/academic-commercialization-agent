"""Independent RP-native wire boundary; offline engineering, not live authority.

The frozen relation-policy wrapper owns final parsing and receipt semantics.
This module owns a detached, single-owner synchronous conversation and compares
the full system, verbatim claim, catalog, accepted call and deterministic read.
Comparison establishes consistency, not proof of tool execution or semantics.

Only the pinned base key/HTTP/accounting primitives and pure helpers are reused.
Calling the old claim/catalog adapters or changing their prompts would bypass
their distinct manifests and hash-bound admission rules, so limited explicit
duplication is intentional. No environment lookup, runner, CLI, retries, repair,
production route or new paid allowance is introduced. Tests intercept inherited
_post with fake-key MockTransport; the class itself is not a network sandbox.
"""
...
TRANSPORT_IDENTITY = "report_evidence_relation_policy_qwen_transport_v1"
...
def relation_policy_qwen_configuration() -> dict:
    ...
    return {
        **result, "method_id": claim_policy.METHOD_ID,
        ...
        "callback_contract": "one_positional_exact_dict_messages_tools_tool_choice",
...
class RelationPolicyQwenLedger(CanaryLedger):
    ...
    def __init__(self, output_dir):
        super().__init__(output_dir, {
            ...
            "configuration": relation_policy_qwen_configuration(),
...
class RelationPolicyQwenFollowupTransport(QwenFollowupTransport):
    ...
    def __init__(self, api_key: str, ledger: RelationPolicyQwenLedger, *, snapshot: ReportEvidenceSnapshot, claim: str):
        if type(ledger) is not RelationPolicyQwenLedger:
            raise CanaryStopped("relation_policy_ledger_required")
        ...
        # The predecessor's computed hash is not an independent frozen anchor.
        # Check literal UTF-8 bytes as well as its exported hash before any
        # request. Bind the checked string once; never append on the wire path.
        if (type(policy.POLICY_APPEND) is not str
                ...
                or policy.POLICY_ID != "report_evidence_relation_policy_v1"):
            ledger.stop("policy_identity_mismatch")
            raise CanaryStopped("policy_identity_mismatch")
        self._policy_append = policy.POLICY_APPEND
        self._claim = claim
        try:
            ...
            for state in (False, True):
                ...
                if (expected.count(claim_policy._OLD_FINAL) != 1
                        or claim_policy._NEW_FINAL in expected or self._policy_append in expected):
                ...
                self._systems[state] = expected.replace(claim_policy._OLD_FINAL, claim_policy._NEW_FINAL, 1) + self._policy_append
    ...
    def __call__(self, request, /):
        """Admit one detached RP callback, reserve final bytes, then dispatch.

        Positional-only exact-key admission prevents accidental reuse of the
        older keyword adapters and silent dropping of caller-supplied fields.
        Policy callback entry has already happened: a refusal here must not
        rewrite its audit into a claim that no local read was delivered.
        """
        ...
        try:
            ...
            body = {"model": MODEL, **detached, "stream": False, "enable_thinking": False,
        ...
        # reserve canonicalizes this same body; _post receives the immutable
        # encoding already bounded above. Re-encoding or mutating after reserve
        # would separate paid intent identity from the actual HTTP content.
