"""One fixed PCQ contrast batch; no production route or inherited authority.

Identity, checked dispatch, receipt and publication patterns are adapted from
CLQ, never imported from its closed runner. The unchanged native claim adapter
and exact ledger own HTTP/accounting. The parent verifies review/CI/live scope.
Preparation labels and scripted answers are local data, never model input.
"""
...
from academic_agent.report_evidence_claim_contrast import load_draft
...
PROTOCOL_IDENTITY = "report_evidence_claim_contrast_qwen_canary_v1"
ROOT = Path(__file__).resolve().parents[2]
FIXED_OUTPUT = "outputs/report_evidence_claim_contrast_qwen_canary_v1"
FIXTURE = "tests/fixtures/report_evidence_claim_contrast.json"
FIXTURE_SHA256 = "bfc9347b28a7d284b8d959c93a038a14f1c15c9adb0d3394c9c6463012141c78"
PROTOCOL = "docs/prereg-2026-09-17-claim-proposition-contrast-qwen-canary.md"
MAX_REQUESTS = 6
MAX_CASE_REQUESTS = 2
CASE_IDS = ("PCQ01", "PCQ02", "PCQ03")
...
IDENTITY_PATHS = tuple(dict.fromkeys((
    ...
    "src/academic_agent/report_evidence_claim_contrast_qwen_canary.py",
    "report_evidence_claim_contrast_canary.py", "tests/test_report_evidence_claim_contrast_qwen_canary.py",
    "src/academic_agent/report_evidence_claim_contrast.py",
    "tests/test_report_evidence_claim_contrast.py",
    "docs/prereg-2026-09-17-claim-proposition-contrast.md",
    "docs/results-2026-09-17-claim-proposition-contrast.md",
    FIXTURE, PROTOCOL,
)))
DEPENDENCIES = (
...
def snapshot_for(case: dict) -> ReportEvidenceSnapshot:
    # Do not promote synthetic controls to verified abstracts or reuse offline
    # snapshot/receipt IDs. Every new conversation is bound to its own case ID.
    return ReportEvidenceSnapshot(report_ref=case["case_id"], sources=(
        SnapshotSource(**case["source"]),))


def load_cases() -> tuple[dict, ...]:
    """Reuse only the exact preparation loader, never its scripted transport/gate."""
    try:
        cases = tuple(case.model_dump(mode="json") for case in load_draft(ROOT / FIXTURE).cases)
        for case, case_id, relation in zip(cases, CASE_IDS, RELATIONS, strict=True):
            ...
            if (case["case_id"] != case_id or case["expected_relation"] != relation
                    or source["source_id"] != "A1" or source["group"] != "academic"
                    or source["publisher"] != "Synthetic controls"
                    or source["source_type"] != "synthetic_control"
                    or source["accessed_date"] != "2026-09-17"):
                raise ValueError("invalid frozen control")
            snapshot = snapshot_for(case)
            catalog = build_catalog(snapshot)
            if (snapshot.sources[0].origin != "unknown" or catalog["returned_count"] != 1
                    or catalog["coverage"] != "complete" or catalog["omitted_count"] != 0
                    or catalog["title_truncation_count"] != 0):
...
def _full_read_call(message, case, snapshot):
    ...
    if (call["function"]["name"] != "read_source" or args.source_id != case["source"]["source_id"]
...
def _serialize_result(result) -> dict:
    ...
    if type(result) is not claim_policy.ClaimRelationFollowupResult:
    ...
    counts = (
        ...
        *audit.callback_bytes, *catalog.callback_bytes,
    )
    ...
    validated = claim_policy.ClaimRelationFollowupResult.model_validate_json(_encoded(observed))
...
def _case_gate(case, snapshot, result, records, ledger):
    ...
    try:
        ...
        delivered = claim_policy.ClaimRelationFollowupResult.model_validate_json(_encoded(observed))
        return _check_case(case, snapshot, delivered, records, ledger)
...
def run_canary(*, expected_commit: str, expected_fixture_sha256: str, authorize_paid: str) -> dict:
    ...
    summary = {
        "mode": "synthetic_claim_contrast_qwen_canary", "protocol_identity": PROTOCOL_IDENTITY,
