"""Source-lock and review the frozen adaptive role-gap v8 AC run.

The provider runner deliberately stopped after one mechanically complete,
production-disconnected AC01-AC08 retrieval run.  This module binds those
...
validated.  It opens no socket, invokes no model, does not inspect AD01-AD08,
and never authorizes production Tool Calling.
"""
...
from openalex_role_gap_live import (
...
EXPECTED_EXECUTED_REVISION = "59b5870614d23c0d9c61e7e398fa363026b6a528"
...
# The exact AC run searched a closure for AC01-AC07 and explicitly abstained
# for AC08.  Freezing the filenames prevents a different valid v8 execution
# from being substituted behind the real run's artifact-index digest.
EXPECTED_CLOSURE_CASE_IDS = CASE_IDS[:-1]
...
EXPECTED_PROVIDER_CANDIDATE_COUNT = 72
EXPECTED_PROVIDER_REJECTION_COUNT = 18
EXPECTED_REVIEW_ROW_COUNT = 64
...
MEASUREMENT_LIMIT = (
    ...
    "AC01-AC08 run. It does not verify full-text source truth, estimate recall, "
    "evaluate AD01-AD08, measure report value or planner-trigger precision, or "
    "authorize production Tool Calling."
)


class RoleGapSourceLock(BaseModel):
    """Study-owner attestation over the exact AC01-AC08 provider bytes."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    schema_version: Literal[1] = 1
    mode: Literal["openalex_adaptive_role_gap_v8_source_artifact_lock"] = (
        "openalex_adaptive_role_gap_v8_source_artifact_lock"
    )
    ...
    def _validate_frozen_identity(self) -> "RoleGapSourceLock":
        ...
        if set(self.source_file_sha256) != set(SOURCE_FILES):
            raise ValueError("source lock must identify every v8 AC source file")
        if self.source_file_sha256.get("artifact-index.json") != (
            EXPECTED_ARTIFACT_INDEX_SHA256
        ):
            raise ValueError("source lock does not identify the frozen v8 AC run")
        if any(
            len(value) != 64
...
def _validate_artifact_index(source_dir: Path) -> dict[str, str]:
    ...
    if _file_sha256(index_path) != EXPECTED_ARTIFACT_INDEX_SHA256:
        raise RoleGapReviewError("artifact-index identity does not match the v8 AC run")
    ...
    if not isinstance(indexed, dict) or set(indexed) != set(INDEXED_SOURCE_FILES):
        raise RoleGapReviewError("artifact index does not cover the exact v8 AC files")
    if any(
...
def _validate_complete_execution(
    ...
    if execution.overall_state != "completed":
        raise RoleGapReviewError("only the completed AC01-AC08 run may be locked")
    if (
...
def _validate_frozen_totals(
    ...
    if tuple(case.spec.case_id for case in manifest.cases) != CASE_IDS:
        raise RoleGapReviewError("manifest does not contain AC01-AC08 exactly")
    if tuple(portfolio.case_id for portfolio in execution.portfolios) != CASE_IDS:
        raise RoleGapReviewError("execution portfolios are not AC01-AC08")
...
def _validate_route_journals(
    ...
    if observed_names != expected_names:
        raise RoleGapReviewError("route journals do not cover AC01-AC08 exactly")
...
def _validate_case_portfolios(
    ...
    if observed_names != expected_names:
        raise RoleGapReviewError("case portfolios do not cover AC01-AC08 exactly")
...
candidates from one exact, completed and production-disconnected AC01-AC08
...
All six frozen gates are conjunctive. Passing AC permits only a separately
pre-registered AD01-AD08 evaluation. It never authorizes production Tool
Calling.
"""
...
def _packet_manifest(
    ...
    *,
    ...
    return {
        "schema_version": 2,
        "mode": "openalex_adaptive_role_gap_v8_source_review_packet",
...
def summarize_packet(
    ...
    return {
        "schema_version": 1,
        "mode": "openalex_adaptive_role_gap_v8_source_review_result",
        "cohort": "development",
        "production_connected": False,
        "report_workflow_connected": False,
        "planner_trigger_connected": False,
        ...
        "ad_evaluation_eligible": decision == "pass",
        "production_connection_authorized": False,
        "remaining_production_blockers": (
            [
                "separately_preregistered_ad_evaluation",
            ...
            if decision == "pass"
                ...
                "adaptive_role_gap_v8_candidate_value",
                "planner_trigger_precision",
                "disabled_path_regression",
                "report_value",
            ]
...
def _parser() -> argparse.ArgumentParser:
    ...
    lock = commands.add_parser("lock", help="bind the exact v8 AC live output")
