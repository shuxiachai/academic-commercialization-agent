...
from academic_agent.report_evidence_final_json_qwen_transport import (
    FinalJsonQwenFollowupTransport, FinalJsonQwenLedger, TRANSPORT_IDENTITY, final_json_configuration,
)
...
def request_kwargs(mode="none"):
    return {"messages": [{"role": "user", "content": "Synthetic question; return JSON."}],
...
@pytest.fixture
def ledger(tmp_path):
    return FinalJsonQwenLedger(tmp_path / "final-json")
...
@pytest.fixture
def snapshot():
    return ReportEvidenceSnapshot(report_ref="final-json-wire-offline", sources=tuple(
...
def test_direct_read_and_missing_text_finalize_without_lookup(snapshot, ledger, mock_http, tool_spy, source_id, missing):
    ...
    result = run_policy_followup(snapshot, "Inspect a registered synthetic record", transport=FinalJsonQwenFollowupTransport(KEY, ledger))
    assert_wire_records(mock_http, ledger, intents)
...
def test_zero_literal_hit_reaches_none_abstention(snapshot, ledger, mock_http, tool_spy):
    ...
    result = run_policy_followup(snapshot, "Inspect the control", transport=FinalJsonQwenFollowupTransport(KEY, ledger))
    body = json.loads(mock_http["requests"][1].content)
...
def test_modified_schema_is_not_silently_rebuilt(ledger, mock_http, kind):
    ...
    with pytest.raises(frozen.CanaryStopped, match="invalid_stage_toolset"):
        FinalJsonQwenFollowupTransport(KEY, ledger)(**{**request_kwargs("auto"), "tools": tools})
...
def test_enum_order_is_preserved_and_caller_mutation_detached(ledger, mock_http):
    ...
    result = FinalJsonQwenFollowupTransport(KEY, ledger)(**supplied)
...
def test_pinned_wire_options_and_separate_manifest_identity(ledger, mock_http, monkeypatch):
    ...
    FinalJsonQwenFollowupTransport(KEY, ledger)(**request_kwargs())
    ...
    assert {key: value for key, value in config.items() if key not in {
        "tool_choice_by_stage", "final_tools_wire", "response_format_by_stage", "final_json_keyword_guard",
    }} == original
    assert config["response_format_by_stage"] == {
        "initial": "omitted", "read": "omitted", "final": {"type": "json_object"},
    }
    assert config["final_json_keyword_guard"] == {
        "roles": ["system", "user"], "pattern": r"\bJSON\b",
        "flags": ["ASCII", "IGNORECASE"], "repair": False,
    }
    changed = final_json_configuration()
    changed["tool_choice_by_stage"]["final"] = "auto"
    assert final_json_configuration()["tool_choice_by_stage"]["final"] == "none"
...
def test_invalid_request_fails_before_durable_intent(ledger, mock_http, kind):
    ...
    with pytest.raises(frozen.CanaryStopped, match=reason):
        FinalJsonQwenFollowupTransport(KEY, ledger)(**supplied)
    assert not ledger.records and not mock_http["requests"]
...
def test_occupied_output_and_old_ledger_are_rejected(ledger, tmp_path, mock_http):
    ...
    with pytest.raises(frozen.CanaryStopped, match="output_creation_failed_or_occupied"):
        FinalJsonQwenLedger(ledger.output_dir)
    ...
    with pytest.raises(frozen.CanaryStopped, match="final_json_ledger_required"):
        FinalJsonQwenFollowupTransport(KEY, old)
...
def test_invalid_dedicated_key_never_falls_back(ledger, mock_http, key):
    ...
    with pytest.raises(frozen.CanaryStopped, match="invalid_dedicated_key"):
        FinalJsonQwenFollowupTransport(key, ledger)
    assert not ledger.records and not mock_http["requests"]
...
@pytest.mark.parametrize("messages", [
    ...
    [{"role": "assistant", "content": "Return JSON."}],
    [{"role": "tool", "content": '{"format":"JSON"}'}],
    [{"role": "user", "content": "JSONify JSONL xJSON JSON_ JSON2"}],
    [{"role": "user", "content": "ＪＳＯＮ"}],
    [{"role": "user", "content": ["JSON"]}],
    [{"role": "user", "content": {"format": "JSON"}}],
    [{"role": "system", "content": None}, {"role": "assistant", "content": "JSON"}],
    [{"role": "developer", "content": "JSON"}],
    [{"role": "user", "JSON": "in a key only", "content": "no format"}],
], ids=["missing", "assistant", "tool", "substring", "fullwidth", "list", "object", "null", "developer", "key"])
def test_final_json_keyword_rejected_before_reservation(ledger, mock_http, messages):
    """Only a string system/user ASCII JSON word can authorize this wire mode."""
    original = deepcopy(messages)
    transport = FinalJsonQwenFollowupTransport(KEY, ledger)
    with pytest.raises(frozen.CanaryStopped, match="final_json_keyword_required"):
        transport(messages=messages, tools=[], tool_choice="none")
    assert messages == original
    assert not ledger.records and not mock_http["requests"]
    assert ledger.summary()["request_count"] == 0
    assert Decimal(ledger.summary()["budget_consumed_usd"]) == 0
    assert events(ledger) == [{"event": "batch_stopped", "reason": "final_json_keyword_required"}]
    with pytest.raises(frozen.CanaryStopped, match="batch_already_stopped"):
        transport(**request_kwargs())
    assert not mock_http["requests"]
...
@pytest.mark.parametrize("role,content", [
    ("system", "Return a JSON object."), ("user", "(json)"),
    ("system", "Mixed JsOn."), ("user", "中文JSON。"),
], ids=["system", "user", "case", "ascii_boundary"])
def test_final_json_keyword_accepts_only_frozen_rule_without_prompt_rewrite(ledger, mock_http, role, content):
    """The adapter validates supplied instructions, never adds a hidden repair prompt."""
    messages = [{"role": role, "content": content}]
    intents = script(mock_http, ledger, final())
    FinalJsonQwenFollowupTransport(KEY, ledger)(messages=messages, tools=[], tool_choice="none")
    assert_wire_records(mock_http, ledger, intents)
    body = json.loads(mock_http["requests"][0].content)
...
@pytest.mark.parametrize("tools", [core.tool_definitions(), read_tools()], ids=["initial", "read"])
def test_nonfinal_has_no_json_mode_or_keyword_requirement(ledger, mock_http, tools):
    """Final JSON mode cannot leak into native lookup/read declarations."""
    messages = [{"role": "user", "content": "No output format specified."}]
    intents = script(mock_http, ledger, final())
    FinalJsonQwenFollowupTransport(KEY, ledger)(messages=messages, tools=tools, tool_choice="auto")
    assert_wire_records(mock_http, ledger, intents)
    body = json.loads(mock_http["requests"][0].content)
    assert body["messages"] == messages and body["tools"] == tools
    assert body["tool_choice"] == "auto" and "response_format" not in body


def test_new_ledger_reuses_frozen_accounting_not_stage_identity(ledger, tmp_path, mock_http):
    """A fresh manifest cannot bypass or replace the original reserve/finish accounting."""
    from academic_agent.report_evidence_stage_qwen_transport import StageQwenLedger

    assert FinalJsonQwenLedger.reserve is frozen.CanaryLedger.reserve
    assert FinalJsonQwenLedger.finish is frozen.CanaryLedger.finish
    stage = StageQwenLedger(tmp_path / "old-stage")
    before = (stage.output_dir / "manifest.json").read_bytes()
    with pytest.raises(frozen.CanaryStopped, match="final_json_ledger_required"):
        FinalJsonQwenFollowupTransport(KEY, stage)
    assert (stage.output_dir / "manifest.json").read_bytes() == before

    class OtherLedger(FinalJsonQwenLedger):
        pass

    with pytest.raises(frozen.CanaryStopped, match="final_json_ledger_required"):
        FinalJsonQwenFollowupTransport(KEY, OtherLedger(tmp_path / "custom"))
    with pytest.raises(TypeError):
        FinalJsonQwenLedger(tmp_path / "arbitrary", {"live_authorization": True})
    assert not mock_http["requests"] and not ledger.records
