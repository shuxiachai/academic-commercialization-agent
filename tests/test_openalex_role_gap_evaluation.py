...
def test_protocol_dry_run_locks_adaptive_options_without_live_authority(
    ...
    assert result["evaluation_runner_maximum_requests"] == 16
    assert result["evaluation_runner_maximum_soft_stop_usd"] == 0.02
...
def test_complete_search_routes_reach_all_write_once_boundaries(tmp_path):
    ...
    assert manifest["cohort"] == "unseen"
    assert execution["cohort"] == "unseen"
    assert manifest["mode"] == "openalex_adaptive_role_gap_v8_ad_manifest"
    assert execution["mode"] == "openalex_adaptive_role_gap_v8_ad_execution"
    assert index["cohort"] == "unseen"
    assert index["mode"] == "openalex_adaptive_role_gap_v8_ad_artifact_index"
...
def test_request_latency_reaches_journal_csv_and_provider_summary(tmp_path):
    ...
    first_journal = (
        output / "lane-executions" / "AD01--anchor_search.json"
...
def test_development_cohort_fails_before_adapter_and_output(tmp_path):
    """The AD entrypoint must reject a development replay before authority."""
    output = tmp_path / "wrong-cohort"
    factory = RecordingFactory(output)

    with pytest.raises(live.OpenAlexRoleGapEvaluationError, match="only the unseen"):
        live.execute_unseen_study(
            cohort="development",
            output_dir=output,
            soft_stop_usd=0.02,
            acknowledge_anonymous_daily_budget=True,
...
def test_fixture_drift_fails_before_adapter_and_output(tmp_path, monkeypatch):
    """Changed fixture bytes must fail before spending authority can exist."""
    output = tmp_path / "fixture-drift"
    fixture = tmp_path / "challenge.json"
    fixture.write_bytes(DEFAULT_FIXTURE_PATH.read_bytes() + b" ")
    factory = RecordingFactory(output)
    original_loader = live.load_frozen_cases

    def load_drifted(cohort):
        return original_loader(cohort, fixture)

    monkeypatch.setattr(live, "load_frozen_cases", load_drifted)

    with pytest.raises(OpenAlexRoleGapPreflightError, match="fixture byte identity"):
        live.execute_unseen_study(
            output_dir=output,
            soft_stop_usd=0.02,
            acknowledge_anonymous_daily_budget=True,
...
def test_invalid_authorization_fails_before_adapter_and_output(
    ...
    with pytest.raises(live.OpenAlexRoleGapEvaluationError):
        live.execute_unseen_study(
            output_dir=output,
...
def test_configured_key_fails_before_adapter_and_output(tmp_path, monkeypatch):
    ...
    with pytest.raises(live.OpenAlexRoleGapEvaluationError, match="refuses"):
        live.execute_unseen_study(
            output_dir=output,
            soft_stop_usd=0.02,
            acknowledge_anonymous_daily_budget=True,
...
def test_implementation_hash_drift_fails_before_factory_and_output(
    ...
    with pytest.raises(live.OpenAlexRoleGapEvaluationError, match="identity drifted"):
        live.execute_unseen_study(
            output_dir=output,
            soft_stop_usd=0.02,
            acknowledge_anonymous_daily_budget=True,
...
def test_soft_stop_preserves_anchor_and_route_without_fabricating_portfolio(
    ...
    assert factory.calls == [("AD01", "anchor_search", None)]
    ...
    assert (output / "lane-executions" / "AD01--anchor_search.json").is_file()
    assert (output / "route-executions" / "AD01.json").is_file()
    assert not (output / "case-portfolios" / "AD01.json").exists()
    provider_rows = _read_csv(output / "provider-rows.csv")
...
def test_production_worker_does_not_import_role_gap_components():
    ...
    assert "openalex_role_gap_evaluation" not in worker
