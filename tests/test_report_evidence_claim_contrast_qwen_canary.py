"""PCQ controls at the real claim wrapper/tool/HTTP/journal boundary, never live.

Adapted from CLQ's frozen tests without importing or executing its runner.
These controls use new PCQ identities and inspect actual intercepted dispatch.
"""
...
import report_evidence_claim_contrast_canary as cli
from academic_agent import report_evidence_claim_contrast_qwen_canary as canary
from academic_agent.report_evidence_claim_qwen_transport import ClaimQwenLedger

KEY = "sk-pcq-offline-invented-not-a-credential"
COMMIT = "a" * 40
...
def provider(request):
    ...
    case = next(case for case in canary.load_cases() if case["claim"] == claim)
    relation = case["expected_relation"]
    ids = [] if relation == "insufficient" else [delivered["evidence_id"]]
    answer = {"supported": "The record gives one resistance observation, without durability evidence.",
              "refuted": "This record explicitly contains no resistance measurement.",
              "insufficient": "The physical resistance cannot be determined from this measurement-free record."}[relation]
    assert answer != case["scripted_answer"]  # Live success never requires the rehearsal answer.
...
import report_evidence_claim_contrast_canary
from academic_agent import report_evidence_claim_contrast_qwen_canary as runner
assert "dotenv" not in sys.modules
assert "academic_agent.report_evidence_claim_qwen_canary" not in sys.modules
...
def test_static_closure_and_exact_ledger_are_bound(offline):
    ...
    pending = ["src/academic_agent/report_evidence_claim_contrast_qwen_canary.py", "report_evidence_claim_contrast_canary.py"]
    seen = set()
    while pending:
        name = pending.pop()
        if name in seen:
    ...
    assert "src/academic_agent/report_evidence_claim_qwen_canary.py" not in seen
    assert {canary.FIXTURE, canary.PROTOCOL, "pyproject.toml", "uv.lock", ".gitattributes",
            ...
            "tests/test_report_evidence_claim_contrast_qwen_canary.py"} <= set(canary.IDENTITY_PATHS)
...
def test_lf_comparison_does_not_normalize_fixture_authority(offline):
    ...
    name = "src/academic_agent/report_evidence_claim_contrast_qwen_canary.py"
    path = offline["repo"] / name
...
@pytest.mark.parametrize("defect", ["order", "relation", "source_id", "group", "publisher",
                                  "source_type", "accessed_date", "long_title"])
def test_preparation_projection_cannot_expand_live_control_scope(offline, monkeypatch, defect):
    """Even a damaged loader projection cannot silently change this batch's fixed controls."""
    draft = canary.load_draft(offline["repo"] / canary.FIXTURE)
    cases = list(draft.cases)
    if defect == "order":
        cases.reverse()
    elif defect == "relation":
        cases[0] = cases[0].model_copy(update={"expected_relation": "supported"})
    else:
        changed = {"source_id": "A2", "group": "market", "publisher": "Other",
                   "source_type": "abstract", "accessed_date": "2020-01-01", "long_title": "x" * 300}[defect]
        field = "title" if defect == "long_title" else defect
        cases[0] = cases[0].model_copy(update={"source": cases[0].source.model_copy(update={field: changed})})
    monkeypatch.setattr(canary, "load_draft", lambda path: draft.model_copy(update={"cases": tuple(cases)}))
    with pytest.raises(canary.CanaryStopped, match="fixture_invalid_or_unavailable"):
        canary.load_cases()
    no_effects(offline)


def test_exact_preparation_loader_and_synthetic_origins_survive(offline):
    """No CLQ projection may promote synthetic source_type/origin or reuse offline receipts."""
    cases = canary.load_cases()
    draft = canary.load_draft(offline["repo"] / canary.FIXTURE)
    assert cases == tuple(case.model_dump(mode="json") for case in draft.cases)
    assert draft.status == "draft_before_llm_review"
    assert cases[0]["source"] == cases[1]["source"]
    assert tuple(case["expected_relation"] for case in cases) == ("insufficient", "refuted", "supported")
    snapshots = [canary.snapshot_for(case) for case in cases]
    assert len({snapshot.snapshot_hash for snapshot in snapshots}) == 3
    for case, snapshot in zip(cases, snapshots, strict=True):
        assert snapshot.report_ref == case["case_id"]
        assert snapshot.sources[0].origin == "unknown"
        assert snapshot.sources[0].source_type == "synthetic_control"
        assert snapshot.sources[0].summary == case["source"]["summary"]


@pytest.mark.parametrize("flag", ["--output-dir", "--force", "--resume", "--case", "--api-key", "--authorize", "--retry", "--model"])
...
@pytest.mark.parametrize("value", ["report_evidence_catalog_qwen_canary_v1", "report_evidence_claim_qwen_canary_v1",
                                  "claim_proposition_contrast_v1", KEY, "", None, 1])
...
def test_real_three_cases_six_http_shared_exact_ledger_and_separate_authority(offline, monkeypatch, capsys):
    ...
    assert summary["batch_passed"] is summary["mechanical_passed"] is summary["label_match_passed"] is True
    ...
    for index, (request, intent, record, setup) in enumerate(zip(
        ...
        for field in ("proposition_kind", "reference_rationale", "scripted_answer"):
            assert field.encode() not in request.content
        for case in cases:
            for field in ("reference_rationale", "scripted_answer"):
                assert case[field] not in request.content.decode("ascii")
        ...
        if index % 2 == 0:
            ...
            assert cases[index // 2]["source"]["summary"] not in request.content.decode()
        else:
            ...
            assert delivered["status"] == "ok" and delivered["text"] == cases[index // 2]["source"]["summary"]
    ...
    for index, case in enumerate(cases):
        ...
        assert result["state"] == ("abstained" if index == 0 else "answered_with_evidence")
        ...
        assert result["model_assessment"]["supporting_evidence_ids"] == ([] if index == 0 else audit["usable_read_ids"])
    ...
    assert set(path.name for path in offline["output"].iterdir()) == {*SETUP, "events.jsonl", "summary.json",
                                                                 "PCQ01.json", "PCQ02.json", "PCQ03.json"}
...
def test_wrapper_failure_after_transport_acceptance_stops_actual_http(offline, defect):
    """Two accepted ledger rows cannot buy PCQ02 when the real claim wrapper refuses."""
    def handle(request):
        body = json.loads(request.content)
    ...
    assert summary["unrun_cases"] == ["PCQ02", "PCQ03"] and not summary["mechanical_passed"]
    ...
    result = read_json(offline["output"] / "PCQ01.json")["result"]
    assert result["state"] == "failed" and result["model_assessment"] is None


@pytest.mark.parametrize("case_index,relation", [(0, "refuted"), (0, "supported"), (1, "supported"),
                                              (1, "insufficient"), (2, "refuted"), (2, "insufficient")])
def test_legal_label_mismatch_stops_actual_http(offline, case_index, relation):
    ...
    assert summary["label_match_passed"] is False and summary["batch_passed"] is False
    assert summary["mechanical_passed"] is (case_index == 2)
...
def test_first_native_read_failure_preserves_usage_and_never_dispatches_second(offline, defect):
    ...
    assert summary["label_match_passed"] is None and summary["unrun_cases"] == ["PCQ02", "PCQ03"]
...
def test_gate_checks_full_receipt_original_claim_and_all_audit_layers(offline, monkeypatch, defect):
    ...
    assert summary["unrun_cases"] == ["PCQ02", "PCQ03"]
...
def test_late_wire_gates_reject_consistent_journal_tampering(offline, monkeypatch, defect, reason):
    ...
    assert read_json(offline["output"] / "PCQ01.json")["gate_failure"] == reason
    assert not summary["mechanical_passed"] and summary["label_match_passed"] is None
    assert summary["unknown_usage_requests"] == 0 and summary["unrun_cases"] == ["PCQ02", "PCQ03"]
...
def test_transport_errors_keep_unknown_distinct_and_stop_batch(offline, defect):
    ...
    assert not summary["mechanical_passed"] and summary["unrun_cases"] == ["PCQ02", "PCQ03"]
...
def publication_io(offline, monkeypatch):
    """JQ-derived durability fault harness, targeting only new PCQ publication names."""
    state = {"target": "PCQ01.json", "fault": None, "fd": None, "steps": [], "at_sync": []}
...
@pytest.mark.parametrize("target", ["identity.json", "experiment_manifest.json", "authorization.json",
                                   "PCQ01.json", "PCQ02.json", "PCQ03.json", "summary.json"])
...
def test_publication_failure_never_buys_next_http(offline, publication_io, capsys, target, fault):
    ...
    assert not result["batch_passed"] and KEY not in captured.out + captured.err
    assert not (offline["output"] / target).exists()
    expected = {"PCQ01.json": 2, "PCQ02.json": 4, "PCQ03.json": 6, "summary.json": 6}.get(target, 0)
...
@pytest.mark.parametrize("layer,field", [
    ("claim", "downstream_calls"), ("catalog", "downstream_calls"),
    ("core", "transport_turns"), ("core", "observed_tool_requests"),
    ("core", "tool_attempts"), ("core", "tool_executions"), ("core", "tool_errors"),
    ("core", "no_tools"), ("catalog", "total_count"), ("catalog", "returned_count"),
    ("catalog", "omitted_count"), ("catalog", "title_truncation_count"),
])
def test_raw_boolean_counter_never_becomes_a_serialized_integer(offline, monkeypatch, layer, field):
    """model_copy bypass plus serializer normalization must never authorize case two."""
    original = canary.claim_policy.run_claim_relation_followup

    def damaged(*args, **kwargs):
        result = original(*args, **kwargs)
        audit, catalog = result.audit, result.audit.catalog
        target = {"claim": audit, "catalog": catalog, "core": catalog.core}[layer]
        value = 0 if field == "no_tools" else bool(getattr(target, field))
        changed = target.model_copy(update={field: value})
        if layer == "core":
            changed = catalog.model_copy(update={"core": changed})
        if layer != "claim":
            changed = audit.model_copy(update={"catalog": changed})
        return result.model_copy(update={"audit": changed})

    monkeypatch.setattr(canary.claim_policy, "run_claim_relation_followup", damaged)
    summary = batch()
    assert len(offline["requests"]) == 2
    assert summary["batch_passed"] is False and summary["label_match_passed"] is None
    assert summary["unrun_cases"] == ["PCQ02", "PCQ03"]


@pytest.mark.parametrize("fault", ["timeout", "redirect", "invalid_json", "token_limit", "secret", "escaped_secret"])
def test_additional_native_failures_stop_without_fallback(offline, fault):
    """HTTP failure cannot become zero cost, retry, alternate model, or credential disclosure."""
    def handler(request):
        if fault == "timeout":
            raise httpx.ReadTimeout("private " + KEY)
        if fault == "redirect":
            return response(b"", status=307, headers={"location": "https://invalid.example/"})
        if fault == "invalid_json":
            return response(b'{"broken":')
        if fault == "token_limit":
            return response(payload(native(), usage={
                "prompt_tokens": 16385, "completion_tokens": 20, "total_tokens": 16405}))
        message = native()
        message["content"] = KEY if fault == "secret" else json.dumps(
            KEY).replace("s", "\\u0073")
    ...
    assert summary["unrun_cases"] == ["PCQ02", "PCQ03"]
    assert summary["unknown_usage_requests"] == (1 if fault in {"timeout", "redirect", "invalid_json"} else 0)
    assert KEY not in json.dumps(summary)
    assert KEY not in (offline["output"] / "events.jsonl").read_text(encoding="utf-8")


def test_soft_budget_precedes_next_dispatch_without_changing_frozen_rates(offline, monkeypatch):
    """A near-limit consumed ledger rejects reserve; this is an injected accounting state."""
    original = ClaimQwenLedger.summary

    def near_limit(ledger):
        result = original(ledger)
        if ledger.records:
            result["budget_consumed_usd"] = "0.095"
        return result

    monkeypatch.setattr(ClaimQwenLedger, "summary", near_limit)
    summary = batch()
    assert len(offline["requests"]) == 1
    assert summary["batch_passed"] is False and summary["stop_reason"] == "budget_limit"
    assert summary["unknown_usage_requests"] == 0
    assert Decimal(summary["known_usage_estimated_usd"]) > 0


@pytest.mark.parametrize("target", ["PCQ03.json", "summary.json"])
def test_publication_is_distinct_from_three_mechanical_and_label_observations(
        offline, publication_io, target):
    """Six valid responses do not become a published batch when the final write fails."""
    ...
    assert summary["mechanically_passed_cases"] == summary["label_checks"] == summary["label_matches"] == 3
    assert summary["batch_passed"] is False
    assert summary["stop_reason"] == "persistence_failed"
    assert summary["summary_persisted"] is (target != "summary.json")


def test_success_flags_cannot_override_cli_batch_failure(offline, monkeypatch, capsys):
    """CLI success is the combined batch contract, not label/serialization success alone."""
    monkeypatch.setattr(cli, "run_canary", lambda **kwargs: {
        "batch_passed": False, "mechanical_passed": True, "label_match_passed": True,
        "summary_persisted": True})
    assert cli.main(arguments(paid=True)) == 1
    assert json.loads(capsys.readouterr().out)["batch_passed"] is False
    no_effects(offline)
