...
import openalex_role_gap_review as review
...
def _all_role_signal_text(case: PreparedRoleGapCase) -> str:
    ...
    return f"This AC08 anchor contains every frozen role signal: {'; '.join(phrases)}."
...
def _candidate(
    ...
    summary = (
        ...
        if case.spec.case_id == "AC08" and lane_id == "anchor_search" and index == 0
        else (
...
def _source(tmp_path: Path, monkeypatch) -> Path:
    ...
    assert artifact.route_executions[-1].decision.action == "abstain"
    ...
    monkeypatch.setattr(
        ...
        {**{case_id: 5 for case_id in review.CASE_IDS[:-1]}, "AC08": 3},
    )
    return source
...
def _lock(source: Path, tmp_path: Path) -> Path:
    ...
    review.create_source_lock(
        ...
        authorized_at=datetime(2026, 9, 2, 12, 0, tzinfo=UTC),
    )
    return path
...
def _complete_labels(
    ...
    *,
    ...
    for row in rows:
        ...
        elif strategy == "routing" and case_number <= 3 and (
            "anchor_search" in lanes
        ):
            supported = roles
        elif strategy == "closure_value" and case_number <= 4 and lanes == (
            "role_closure",
        ):
            relevant, novel, supported = "NO", "N/A", []
        elif strategy == "human_coverable" and case_number <= 3:
            supported = [roles[0]]
        elif strategy == "coverability_gain" and case_number <= 6 and (
            "anchor_search" in lanes
        ):
            supported = roles
...
def test_source_lock_and_packet_preserve_rows_without_adaptive_provenance(
    ...
    assert manifest["ad_cohort_opened"] is False
...
def test_blank_packet_is_incomplete_and_keeps_route_hidden(tmp_path, monkeypatch):
    ...
    assert result["ad_evaluation_eligible"] is False
...
def test_complete_review_passes_six_gates_then_joins_route(tmp_path, monkeypatch):
    ...
    assert result["ad_evaluation_eligible"] is True
...
def test_production_worker_and_provider_adapter_do_not_import_review_path():
    ...
    for path in production_files:
        assert "openalex_role_gap_review" not in path.read_text(encoding="utf-8")
