...
import report_evidence_final_json_canary as cli
from academic_agent import report_evidence_followup as core
from academic_agent import report_evidence_final_json_qwen_canary as canary
from academic_agent.report_evidence_final_json_qwen_transport import FinalJsonQwenLedger, final_json_configuration

KEY = "sk-final-json-offline-synthetic-only"
COMMIT = "a" * 40
...
def native_read(source_id="A9", call_id="native_read", length=1500):
...
def provider(lookup=False):
    ...
    def handle(request):
        ...
        source_id = "M6" if negative else "A9"
        prefix = "jq02" if negative else "jq01"
        last = body["messages"][-1]
        if last["role"] != "tool":
            if lookup:
                query = "river membrane" if negative else "piezoelectric vibration sensor"
        ...
        message = (final(status="abstained", answer="The saved text is unavailable; I abstain.") if negative
                   else final([result["evidence_id"]], answer="18 Hz; no field deployment or lifetime testing was performed."))
...
@pytest.mark.parametrize("name", ["src/academic_agent/__init__.py", "src/academic_agent/report_evidence_qwen_transport.py",
                                  "src/academic_agent/report_evidence_final_json_qwen_canary.py", "pyproject.toml"],
...
def test_cli_identity_failures_precede_all_effects(offline, monkeypatch, capsys, defect):
    ...
    if defect == "ackhash":
        ...
        changed = path.read_bytes().replace(b"18 Hz", b"99 Hz")
        path.write_bytes(changed)
        offline["committed"][canary.FIXTURE] = changed
    elif defect == "untracked":
        del offline["committed"]["report_evidence_final_json_canary.py"]
    else:
        def git(*operation):
            ...
            if defect == "dirty" and operation[0] == "status":
                return b" M report_evidence_final_json_canary.py\n"
...
def test_source_crlf_comparison_keeps_distinct_disk_hash(offline):
    ...
    name = "src/academic_agent/report_evidence_final_json_qwen_canary.py"
...
def test_fixture_and_configuration_are_new_and_fixed(offline):
    ...
    assert [(row["case_id"], row["source_id"]) for row in cases] == [("JQ01", "A9"), ("JQ02", "M6")]
    assert cases[0]["summary"] == ("The synthetic piezoelectric vibration sensor showed a resonant frequency of 18 Hz "
                                    "in a laboratory fixture. No field deployment or lifetime testing was performed.")
    ...
    assert config["protocol_identity"] == "report_evidence_final_json_qwen_canary_v1"
    assert config["transport_identity"] == "report_evidence_final_json_qwen_transport_v1"
...
def test_cli_real_policy_tools_http_journal_summary(offline, capsys, lookup):
    ...
    for index, (request, intent, setup) in enumerate(zip(offline["requests"], offline["intents"],
        ...
        if body["tool_choice"] == "none":
            ...
            if len(body["tools"]) == 1:
                ...
                assert ids == (["A9"] if index < count // 2 else ["M6"])
        ...
        assert setup["manifest.json"]["configuration"] == final_json_configuration()
        ...
        assert auth["parent_operator_only"] is True and auth["standing_authorization_source"] == canary.PROTOCOL
    ...
    positive = read_json(offline["output"] / "JQ01.json")
    negative = read_json(offline["output"] / "JQ02.json")
    ...
    for outcome in (positive, negative):
        ...
        expected_tools = [["lookup_sources", "read_source"]]
        if lookup:
            expected_tools += [["read_source"]]
        expected_tools += [[]]
        assert audit["advertised_tools"] == expected_tools
        assert audit["core"]["tool_executions"] == (2 if lookup else 1)
        assert audit["core"]["tool_attempts"] == audit["core"]["tool_executions"]
    assert positive["result"]["evidence_ids"] == positive["result"]["audit"]["core"]["delivered_read_ids"]
    ...
    assert {path.name for path in offline["output"].iterdir()} == {
        "manifest.json", "identity.json", "authorization.json", "events.jsonl", "JQ01.json", "JQ02.json", "summary.json"}
...
def test_first_core_failure_suppresses_actual_second_case_http(offline):
    """A transport-accepted invalid final must prevent even JQ02's first HTTP request."""
    ordinary = provider()
    def handle(request):
        if "annual revenue" in json.loads(request.content)["messages"][1]["content"]:
    ...
    assert all("annual revenue" not in json.loads(request.content)["messages"][1]["content"] for request in offline["requests"])
    assert not summary["passed"] and summary["unrun_cases"] == ["JQ02"]
    ...
    assert read_json(offline["output"] / "JQ01.json")["result"]["audit"]["core"]["terminal_reason"] == "invalid_final_envelope"
    assert not (offline["output"] / "JQ02.json").exists()
...
def test_first_case_failure_never_becomes_positive_or_repair(offline, defect):
    ...
    assert summary["passed"] is False and summary["unrun_cases"] == ["JQ02"]
...
def test_missing_text_case_requires_read_delivery_and_structured_abstention(offline, defect):
    """JQ02 cannot pass from lookup metadata, no tool, unstructured refusal or an answer."""
    ordinary = provider()
    def handle(request):
        body = json.loads(request.content)
...
@pytest.mark.parametrize("filename", ["JQ01.json", "summary.json"])
def test_result_write_failure_retains_observed_usage(offline, monkeypatch, filename):
    """Failed publication is not success or free work, and JQ01 failure stops JQ02."""
    offline["handler"] = provider()
    original = Path.open
    def fail(path, mode="r", *args, **kwargs):
        if path in {offline["output"] / filename, offline["output"] / ("." + filename + ".pending")} and mode == "xb":
    ...
    assert summary["unknown_usage_requests"] == 0 and Decimal(summary["known_usage_estimated_usd"]) > 0
    assert len(offline["requests"]) == (2 if filename == "JQ01.json" else 4)
    assert summary["summary_persisted"] is (filename != "summary.json")
    assert summary["unrun_cases"] == (["JQ02"] if filename == "JQ01.json" else [])
...
def test_journal_failure_stops_batch_and_preserves_received_usage(offline, monkeypatch, phase):
    ...
    summary = batch(offline)
    assert not summary["passed"] and summary["stop_reason"] == "persistence_failed"
    assert summary["unrun_cases"] == ["JQ02"]
...
def test_identity_loss_stops_later_dispatch_without_erasing_usage(offline, monkeypatch, phase):
    ...
    monkeypatch.setattr(canary, "_git", git)
    def handle(request):
        with (offline["repo"] / "src/academic_agent/report_evidence_final_json_qwen_canary.py").open("ab") as stream:
...
def test_http_failures_stop_without_retry_or_secret_diagnostics(offline, capsys, defect):
    ...
    assert cli.main(arguments(offline, paid=True)) == 1
    summary = json.loads(capsys.readouterr().out)
    assert summary["unrun_cases"] == ["JQ02"] and len(offline["requests"]) == 1
    assert summary["unknown_usage_requests"] == (0 if defect == "secret" else 1)
    assert KEY not in json.dumps(summary)
    assert all(KEY not in path.read_text(encoding="utf-8") for path in offline["output"].iterdir())

@pytest.mark.parametrize("defect", [
    "fenced", "prose_fenced", "trailing", "duplicate", "array", "null",
    "extra", "missing", "status", "number", "evidence_type", "empty", "nonfinite",
])
def test_final_json_wire_does_not_repair_or_relax_core_envelope(offline, defect):
    """JSON mode cannot turn a paid malformed final into a pass or buy JQ02."""
    ordinary = provider()
    rejected = []
    ...
    def handle(request):
        ...
        envelope = {"answer": "18 Hz; no field deployment or lifetime testing.",
                    "status": "answered", "evidence_ids": [delivered["evidence_id"]]}
        raw = json.dumps(envelope)
        if defect in {"fenced", "prose_fenced"}:
            raw = ("A synthetic answer.\n" if defect == "prose_fenced" else "") + "```json\n" + raw + "\n```"
        elif defect == "trailing":
            raw += "\nExtra prose."
        elif defect == "duplicate":
            raw = '{"answer":"duplicate",' + raw[1:]
        elif defect in {"array", "null"}:
            raw = "[" + raw + "]" if defect == "array" else "null"
        elif defect == "nonfinite":
            raw = raw.replace('"answered"', "NaN")
        else:
            if defect == "extra":
                envelope["verified"] = True
            elif defect == "missing":
                del envelope["evidence_ids"]
            elif defect == "status":
                envelope["status"] = "verified"
            elif defect == "number":
                envelope["answer"] = 18
            elif defect == "evidence_type":
                envelope["evidence_ids"] = delivered["evidence_id"]
            else:
                envelope["answer"] = ""
            raw = json.dumps(envelope)
        rejected.append(raw)
        return response(payload({"role": "assistant", "content": raw}))

    offline["handler"] = handle
    summary = batch(offline)
    assert summary["passed"] is False and summary["unrun_cases"] == ["JQ02"]
    assert summary["stop_reason"] == "policy_core_failed"
    assert summary["request_count"] == len(offline["requests"]) == 2
    assert not (offline["output"] / "JQ02.json").exists()
    outcome = read_json(offline["output"] / "JQ01.json")
    assert outcome["result"]["state"] == "failed"
    assert outcome["result"]["audit"]["core"]["terminal_reason"] == "invalid_final_envelope"
    assert not outcome["result"]["evidence_ids"]
    finished = [row for row in events(offline["output"]) if row["event"] == "request_finished"]
    assert all(row["reported_usage"] == USAGE and row["protocol_accepted"] is True for row in finished)
    assert finished[-1]["assistant_message"]["content"] == rejected[0]
    assert summary["unknown_usage_requests"] == 0
    assert Decimal(summary["known_usage_estimated_usd"]) > 0
    final_body = json.loads(offline["requests"][-1].content)
    assert final_body["response_format"] == {"type": "json_object"}
    assert final_body["tool_choice"] == "none" and "tools" not in final_body


def test_identity_freeze_includes_new_contract_and_reused_sources(offline):
    """New JQ identity binds executed dependencies, not an SQ/FQ fixture or allowance."""
    required = {
        ".gitattributes", "src/academic_agent/__init__.py", "pyproject.toml", "uv.lock",
        ...
        "report_evidence_final_json_canary.py",
        "tests/test_report_evidence_final_json_qwen_transport.py",
        "tests/test_report_evidence_final_json_qwen_canary.py",
        "docs/prereg-2026-09-16-report-evidence-final-json-qwen.md",
        "tests/fixtures/report_evidence_final_json_qwen_canary.json",
    }
    assert set(canary.IDENTITY_PATHS) == required
    assert canary.FIXTURE_SHA256 == "f0e8233a7b0e0d86783f069d96880ace941cd0ac16dbfa934e723d653febb5ca"
    assert hashlib.sha256((offline["repo"] / canary.FIXTURE).read_bytes()).hexdigest() == canary.FIXTURE_SHA256
    assert_no_side_effects(offline)


@pytest.mark.parametrize("defect", ["hash", "format", "early_format", "tools", "choice", "read_pair"],
                         ids=["hash", "final_mode", "initial_mode", "tools", "choice", "native_pair"])
def test_runner_requires_recorded_wire_and_true_native_read_pair(offline, monkeypatch, defect):
    """A successful core result cannot mask a changed final wire or an invented read pair."""
    original = FinalJsonQwenLedger.finish

    def finish(ledger, ordinal, **kwargs):
        original(ledger, ordinal, **kwargs)
        record = ledger.records[ordinal - 1]
        body = record["request"]
        if defect == "early_format":
            ...
            body["messages"][-2]["tool_calls"][0]["function"]["arguments"] = json.dumps({
                "source_id": "A9", "offset": 1, "length": 1500,
            })
        # A self-consistent hash alone must not satisfy the stage/native gate.
        record["request_sha256"] = hashlib.sha256(canary._encoded(body)).hexdigest()
    ...
    assert summary["passed"] is False and summary["unrun_cases"] == ["JQ02"]
    assert len(offline["requests"]) == 2
    assert summary["stop_reason"] == ("case_gate_failed" if defect == "read_pair" else "stage_wire_observation_failed")
    assert summary["unknown_usage_requests"] == 0
    assert Decimal(summary["known_usage_estimated_usd"]) > 0

@pytest.fixture
def publication_io(offline, monkeypatch):
    """Observe actual file bytes at fsync, including the defective direct-name path."""
    state = {"target": "summary.json", "fault": None, "steps": [], "sync": [], "links": [], "fd": None}
    original_open, original_sync, original_link = Path.open, os.fsync, os.link

    def paths():
        name = state["target"]
        return offline["output"] / name, offline["output"] / ("." + name + ".pending")
    ...
    class Stream:
        ...
        def __exit__(self, *args):
            try:
                ...
                if state["fault"] == "close":
                    raise OSError("private close failure " + KEY)
        ...
        def write(self, raw):
            ...
            if state["fault"] in {"write", "short_write"}:
                ...
                if state["fault"] == "write":
                    raise OSError("private write failure " + KEY)
                return count
        ...
        def flush(self):
            ...
            if state["fault"] == "flush":
                raise OSError("private flush failure " + KEY)
    ...
    def opened(path, mode="r", *args, **kwargs):
        ...
        if path in paths() and mode == "xb":
            return Stream(real, path)
        return real

    def sync(fd):
        if state["fd"] == fd:
            state["steps"].append("fsync")
            final_path, pending_path = paths()
            # Real writes and flush have occurred. No abstract ledger.save mock
            # can satisfy this test; capture the on-disk state at the syscall.
            path = pending_path if pending_path.exists() else final_path
            state["sync"].append({"raw": path.read_bytes(), "path": path,
                                  "final_exists": final_path.exists()})
            if state["fault"] == "fsync":
                raise OSError("private fsync failure " + KEY)
        return original_sync(fd)

    def link(source, destination):
        final_path, _pending_path = paths()
        if destination == final_path:
            state["steps"].append("link")
            state["links"].append((source, destination, source.read_bytes(), destination.exists()))
            if state["fault"] == "link":
                raise OSError("private link failure " + KEY)
        return original_link(source, destination)

    monkeypatch.setattr(Path, "open", opened)
    # Both the frozen direct-name defect and the new runner hit this fsync seam.
    # The runtime implementation changes no module globals; only tests inject IO.
    monkeypatch.setattr(os, "fsync", sync)
    monkeypatch.setattr(canary.os, "fsync", sync)
    monkeypatch.setattr(canary.os, "link", link)
    return state


@pytest.mark.parametrize("target", ["JQ01.json", "JQ02.json", "summary.json"], ids=["positive", "missing", "summary"])
def test_result_fsync_failure_has_no_authoritative_success_on_disk_or_cli(offline, publication_io, capsys, target):
    """After flushed success bytes, a failed fsync must leave only an unpublished candidate."""
    publication_io.update(target=target, fault="fsync")
    offline["handler"] = provider()
    assert cli.main(arguments(offline, paid=True)) == 1
    captured = capsys.readouterr()
    summary = json.loads(captured.out)
    assert summary["passed"] is False and summary["stop_reason"] == "persistence_failed"
    assert summary["summary_persisted"] is (target != "summary.json")
    assert summary["unknown_usage_requests"] == 0 and Decimal(summary["known_usage_estimated_usd"]) > 0
    expected_calls = 2 if target == "JQ01.json" else 4
    assert summary["request_count"] == len(offline["requests"]) == expected_calls
    assert summary["unrun_cases"] == (["JQ02"] if target == "JQ01.json" else [])
    assert not (offline["output"] / target).exists()
    pending = offline["output"] / ("." + target + ".pending")
    assert pending.exists()
    assert publication_io["steps"] == ["write", "flush", "fsync", "close"]
    assert not publication_io["links"]
    observation, = publication_io["sync"]
    assert observation["path"] == pending and observation["final_exists"] is False
    # These success-shaped bytes are a candidate, NOT a persisted result. The
    # authoritative path stays absent; CLI/summary separately report failure.
    assert json.loads(observation["raw"])["passed"] is True
    assert pending.read_bytes() == observation["raw"]
    if target != "summary.json":
        assert summary["cases"][-1]["passed"] is False
        assert summary["cases"][-1]["case_record_persisted"] is False
        assert read_json(offline["output"] / "summary.json") == summary
    else:
        assert all(case["passed"] and case["case_record_persisted"] for case in summary["cases"])
    if target == "JQ01.json":
        assert not (offline["output"] / "JQ02.json").exists()
    finished = [row for row in events(offline["output"]) if row["event"] == "request_finished"]
    assert len(finished) == expected_calls and all(row["reported_usage"] == USAGE for row in finished)
    assert KEY not in captured.out + captured.err
    assert all(KEY not in path.read_text(encoding="utf-8") for path in offline["output"].iterdir())


@pytest.mark.parametrize("target", ["JQ01.json", "JQ02.json", "summary.json"], ids=["positive", "missing", "summary"])
def test_result_publication_is_after_fsync_close_and_matches_cli(offline, publication_io, capsys, target):
    """The public result appears only after real write/flush/fsync/close, with identical bytes."""
    publication_io["target"] = target
    offline["handler"] = provider()
    assert cli.main(arguments(offline, paid=True)) == 0
    summary = json.loads(capsys.readouterr().out)
    assert summary["passed"] is True and summary["summary_persisted"] is True
    assert publication_io["steps"] == ["write", "flush", "fsync", "close", "link"]
    observation, = publication_io["sync"]
    linked, = publication_io["links"]
    assert observation["final_exists"] is linked[3] is False
    assert observation["path"] == linked[0] == offline["output"] / ("." + target + ".pending")
    assert observation["raw"] == linked[2] == (offline["output"] / target).read_bytes()
    assert not linked[0].exists()
    assert read_json(offline["output"] / "summary.json") == summary


@pytest.mark.parametrize("target", ["JQ01.json", "summary.json"], ids=["case", "summary"])
@pytest.mark.parametrize("fault", ["write", "short_write", "flush", "close", "link"])
def test_other_result_publication_faults_keep_final_absent_and_usage_known(
    offline, publication_io, capsys, target, fault,
):
    """Partial writes, close and promotion failures cannot publish a pass or retry."""
    ...
    assert summary["summary_persisted"] is (target != "summary.json")
    assert summary["unknown_usage_requests"] == 0 and Decimal(summary["known_usage_estimated_usd"]) > 0
    assert len(offline["requests"]) == (2 if target == "JQ01.json" else 4)
    assert summary["unrun_cases"] == (["JQ02"] if target == "JQ01.json" else [])
    assert len(publication_io["links"]) == (1 if fault == "link" else 0)
    if target == "JQ01.json":
        assert summary["cases"][0]["case_record_persisted"] is False
        assert not (offline["output"] / "JQ02.json").exists()
        assert read_json(offline["output"] / "summary.json") == summary


@pytest.mark.parametrize("occupied", ["candidate", "destination"])
def test_result_publication_never_overwrites_occupied_names(offline, publication_io, occupied):
    """Exclusive staging and no-clobber linking preserve bytes even if a final name appears."""
    ledger = FinalJsonQwenLedger(offline["output"])
    candidate = offline["output"] / ".summary.json.pending"
    destination = offline["output"] / "summary.json"
    path = candidate if occupied == "candidate" else destination
    sentinel = b"pre-existing result bytes must survive"
    path.write_bytes(sentinel)
    with pytest.raises(canary.CanaryStopped, match="persistence_failed"):
        canary._publish_result(ledger, "summary.json", {"passed": True, "summary_persisted": True})
    assert path.read_bytes() == sentinel
    assert ledger.stop_reason == "persistence_failed"
    assert len(publication_io["links"]) == (1 if occupied == "destination" else 0)
    if occupied == "candidate":
        assert not destination.exists()
    assert not offline["requests"]


def test_candidate_cleanup_failure_does_not_revoke_published_authority(offline, monkeypatch, capsys):
    """A leftover pending hard link is non-authoritative, not a second result or failed publication."""
    candidate = offline["output"] / ".summary.json.pending"
    original = Path.unlink
    def fail(path, *args, **kwargs):
        if path == candidate:
            raise OSError("cannot remove provisional link")
        return original(path, *args, **kwargs)
    ...
    assert summary["passed"] is True and summary["summary_persisted"] is True
    assert read_json(offline["output"] / "summary.json") == summary
    assert candidate.read_bytes() == (offline["output"] / "summary.json").read_bytes()
