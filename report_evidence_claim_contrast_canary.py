"""Verify committed PCQ identity by default; only the parent may dispatch live."""

import argparse
import json

from academic_agent.report_evidence_claim_contrast_qwen_canary import (
...
class _Parser(argparse.ArgumentParser):
    def error(self, message):
        ...
        self.exit(2, "invalid_claim_contrast_canary_arguments\n")
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--authorize-paid", help="Exact PCQ acknowledgement; not independent consent verification.")
    ...
    try:
        ...
        error = "claim_contrast_canary_admission_or_persistence_failed"
    except Exception:  # noqa: BLE001 -- setup/path/provider diagnostics must never expose credentials.
        error = "claim_contrast_canary_setup_failed"
